...
<div class="flex container">
    ...
    <main class="flex-1 p-8 flex flex-col main">
        ...
        <div class="space-y-6 content">
            ...
            <div class="glass p-5 topbar">
                <h2 class="text-2xl font-semibold">Riwayat Lokasi</h2>
            </div>

            <form action="{{ route('riwayat') }}" method="GET" class="flex gap-4 items-center status-bar">
                
                <div class="input-glass px-4 py-2 rounded-lg flex-1 flex items-center gap-2 bg-white/10 backdrop-blur-md border border-white/20 w-full">
                    <span class="text-sm">📅</span>
                    <input type="date" name="start_date" value="{{ request('start_date', '2026-04-01') }}" class="bg-transparent border-none text-white focus:outline-none text-sm cursor-pointer [color-scheme:dark] w-full">
                    <span class="text-xs text-gray-400">s/d</span>
                    <input type="date" name="end_date" value="{{ request('end_date', '2026-04-05') }}" class="bg-transparent border-none text-white focus:outline-none text-sm cursor-pointer [color-scheme:dark] w-full">
                </div>
                ...
                <div class="flex gap-2 w-full sm:w-auto [flex-direction:row]">
                    <button type="submit" class="bg-blue-600 hover:bg-blue-700 text-sm px-4 py-2 rounded-lg transition duration-200 flex-1 sm:flex-initial">
                        Filter
                    </button>

                    <button type="button" class="bg-orange-500 hover:bg-orange-600 px-6 py-2 rounded-lg text-sm font-medium transition duration-200 whitespace-nowrap flex-1 sm:flex-initial">
                        Export CSV
                    </button>
                </div>
            </form>
